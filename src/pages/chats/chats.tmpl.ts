export default `
<div class="chatlist">
  <div class="chatlist__head">
    <p class="chatlist__head__text"><a href="../profile/index.html">Profile ></a></p>
    {{> searchInput}}
  </div>
  <div class="chatlist__items">
    <div class="chatlist__content-box">
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">Image</div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">Image</div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item chatlist__item--active">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
      <div class="chatlist__item">
        <div class="chatlist__item__left">
          <div class="chatlist__item__avatar"></div>
        </div>
        <div class="chatlist__item__mid">
          <div class="chatlist__item__author">Andrew</div>
          <div class="chatlist__item__message">
            <span class="chatlist__item__author-you"> You: </span>
            Image
          </div>
        </div>
        <div class="chatlist__item__right">
          <div class="chatlist__item__date">19:42</div>
          <div class="chatlist__item__unread-count">3</div>
        </div>
      </div>
    </div>
  </div>
</div>
<div class="chat-thread">
  <div class="chat-thread__empty" style="display: none">
    Choose a chat to send a message
  </div>
  <div class="chat-thread__head">
    <div class="chat-thread__head__left">
      <div class="chatlist__item__avatar"></div>
      <div class="chatlist__item__author">Vadim</div>
    </div>
    <div class="chat-thread__head__right">
      <div class="chat-thread__head__settings">
        <div class="chat-thread__head__settings__button">
          <span class="chat-thread__head__settings__button__dot"></span
          ><span class="chat-thread__head__settings__button__dot"></span
          ><span class="chat-thread__head__settings__button__dot"></span>
        </div>
      </div>
    </div>
  </div>
  <div class="chat-thread__content">
    <div class="chat-thread__content__overview-date">18th of June</div>
    <div class="chat-thread__messages">
      <div class="chat-thread__message chat-thread__message--you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--not-you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--not-you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--not-you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--not-you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--not-you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
        </div>
      </div>
      <div
        class="
          chat-thread__message
          chat-thread__message--you
          chat-thread__message--sent
        "
      >
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
          <div class="chat-thread__message__status">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                class="second-check"
                d="M2 12L7.25 17C7.25 17 8.66939 15.3778 9.875 14M16 7L12.5 11"
                stroke="black"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                class="first-check"
                d="M8 12L13.25 17L22 7"
                stroke="black"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
          </div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--not-you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__date">16:34</div>
      </div>
      <div
        class="
          chat-thread__message
          chat-thread__message--you
          chat-thread__message--delivered
        "
      >
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
          <div class="chat-thread__message__status">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                class="second-check"
                d="M2 12L7.25 17C7.25 17 8.66939 15.3778 9.875 14M16 7L12.5 11"
                stroke="black"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                class="first-check"
                d="M8 12L13.25 17L22 7"
                stroke="black"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
          </div>
        </div>
      </div>
      <div class="chat-thread__message chat-thread__message--not-you">
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__date">16:34</div>
      </div>
      <div
        class="
          chat-thread__message
          chat-thread__message--you
          chat-thread__message--seen
        "
      >
        <div class="chat-thread__message__content">Hello!</div>
        <div class="chat-thread__message__bottom-line">
          <div class="chat-thread__message__date">16:34</div>
          <div class="chat-thread__message__status">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                class="second-check"
                d="M2 12L7.25 17C7.25 17 8.66939 15.3778 9.875 14M16 7L12.5 11"
                stroke="black"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                class="first-check"
                d="M8 12L13.25 17L22 7"
                stroke="black"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="chat-thread__actions">
    <div class="chat-thread__actions__attach">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        xmlns:xlink="http://www.w3.org/1999/xlink"
        version="1.1"
        id="Layer_1"
        x="0px"
        y="0px"
        viewBox="0 0 280.067 280.067"
        style="
          enable-background: new 0 0 280.067 280.067;
          width: 25px;
          height: 25px;
        "
        xml:space="preserve"
      >
        <g>
          <path
            style="fill: grey"
            d="M149.823,257.142c-31.398,30.698-81.882,30.576-113.105-0.429   c-31.214-30.987-31.337-81.129-0.42-112.308l-0.026-0.018L149.841,31.615l14.203-14.098c23.522-23.356,61.65-23.356,85.172,0   s23.522,61.221,0,84.586l-125.19,123.02l-0.044-0.035c-15.428,14.771-40.018,14.666-55.262-0.394   c-15.244-15.069-15.34-39.361-0.394-54.588l-0.044-0.053l13.94-13.756l69.701-68.843l13.931,13.774l-83.632,82.599   c-7.701,7.596-7.701,19.926,0,27.53s20.188,7.604,27.88,0L235.02,87.987l-0.035-0.026l0.473-0.403   c15.682-15.568,15.682-40.823,0-56.39s-41.094-15.568-56.776,0l-0.42,0.473l-0.026-0.018l-14.194,14.089L50.466,158.485   c-23.522,23.356-23.522,61.221,0,84.577s61.659,23.356,85.163,0l99.375-98.675l14.194-14.089l14.194,14.089l-14.194,14.098   l-99.357,98.675C149.841,257.159,149.823,257.142,149.823,257.142z"
          />
        </g>
      </svg>
    </div>
    <div class="chat-thread__actions__input">
      <textarea type="text" placeholder="Enter text" rows="2" name="message"></textarea>
    </div>
    <div class="chat-thread__actions__send">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        fill-rule="evenodd"
        clip-rule="evenodd"
      >
        <path
          fill="white"
          d="M21.883 12l-7.527 6.235.644.765 9-7.521-9-7.479-.645.764 7.529 6.236h-21.884v1h21.883z"
        />
      </svg>
    </div>
  </div>
</div>
`;
